import React, { useEffect, useState } from 'react'
import SummaryApi from '../common'
import moment from 'moment'
import displayUSDCurrency from '../helpers/displayCurrency'


const OrderPage =  () => {
const [data, setData] = useState([])
const fetchOrderDetails = async () => {
  const  response = await fetch(SummaryApi.getOrder.url,{
    method : SummaryApi.getOrder.method,
    credentials : 'include'
  })


  const responseData = await response.json()
  setData(responseData.data)

  console.log('order list', responseData)

}

  useEffect(() => {
    fetchOrderDetails()
  }, [])

  return (
    <div>
     {!data[0] && (
      <p>No Order available</p>
     )}
     <div className=' border '>
      {
        data.map((item, index) => {
          return(
             <div className='' key = {item.userId+index}>
               <p className='p-2 font-medium text-medium'>{moment(item.createdAt).format('LL') }  </p>
               <div className='border px-2 grid sm:grid-flow-col justify-between '>

                <div className='grid gap-2'> {
                   item?.productDetails.map((product, index) => {
                return(
                  <div key = {product.productId+index}
                  className='flex gap-3'> 
                
                  <img 
                  src = {product.image[0]}
                  className='w-28 h-28 bg-white object-scale-down p-2'
                  />
                  <div>
                  <div className='font-medium text-lg text-ellipsis line-clamp-1'>{product.name}</div>
                  <div className='flex items-center gap-3 mt-3 ' >
                     <div className='text-lg text-red-500'>{displayUSDCurrency(product.price)}</div>
                  <p>Quantity : {product.quantity}</p>
                    </div>
                  </div>
                  </div>
                  
              )
              }
                  )
                  }
              </div>
              <div className='flex flex-col gap-5'>

           <div> 
            <div className='font-semibold'>Payment Details : </div>
             <p>payment method : {item.paymentDetails.payment_method_types} </p>
             <p>Payment status : {item.paymentDetails.payment_status}</p>
           </div>
           <div>
            <div className='font-semibold'>Shipping Details : </div>
             {item.shipping_options.map((shipping, index) => {
                 return(
                  <div key = {shipping.shipping_rate}>
                  <p>shipping amount : {displayUSDCurrency(shipping.shipping_amount)}</p>
                  </div>
                 )
             })}
             </div>

             <div>
             <p className='p-4 font-bold'>Total Amount : {displayUSDCurrency(item.totalAmount)}</p>
             </div>
                </div>
                </div>
            
             </div>
         
          )})
        }

    </div>
    </div>
  ) }
export default OrderPage